...
_rm()
{
	sleep $TimeDelay;
	if [ "$iPath/" = "$2/$1" ] ; then
		echo -e "[${red}Alert${normal} ] ${white}Deleting path == installing path {normal}, ${yellow}delete aborted${normal}.";
	elif [ -f "$2/$1" ] ; then 
		rm "$2/$1";
	elif [ -d "$2/$1" ] ; then
		rm -r "$2/$1";
	...
	if [ $(isCompressed $2) = 0 ] ; then 
		...
		if [ $? = '0' ] ; then
			...
			if [[ "$result" = "0" ]] ; then
				...
				echo -e "[make  ] ${white}$4${normal}, Installing ${red}FAILED${normal}.";
			fi		
		fi		
	else 
		echo -e "[Path  ] $iPath";
...
hlp()
{
	...
	echo -e "#         ${red}●${normal}   | | ${red}●${normal}   ${red}●${normal}         ${red}●${normal}         ${red}●${normal} | ${red}●${normal}     ${red}●${normal}            |";
